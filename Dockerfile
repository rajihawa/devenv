...
# Neovim
RUN apk add neovim git ripgrep fd gcc musl-dev curl wget gzip unzip bash make
...
# VS Code extensions
RUN cd /root/.vscode/cli/serve-web/${COMMIT}/bin \
...
# run code server
CMD code --verbose -a /app serve-web --without-connection-token --accept-server-license-terms --host 0.0.0.0
